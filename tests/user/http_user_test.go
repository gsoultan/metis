...
func setupHTTPTestService(t *testing.T) (services.ServiceFacade, http.Handler) {
	...
	handlerFactory := handlersimpl.NewNodeHandlerFactory(engine, taskSvc, jobSvc, externalTaskSvc, decisionSvc, repo.Subscription(), service_impl.NewAuditWriter(repo.Audit()))
	engine.Apply(
