...
| `METIS_PLATFORM_ADMINS` | The **platform administrators**: account ids, comma-separated, of the administrators who may install, import, switch on or off and remove connector manifests on an installation with more than one organization. A manifest is installation-wide — a step in any organization that names its key runs it, with that organization's credentials — and roles are global, so the administrator role alone would let one organization's administrator change what every other runs. Unset on such an installation, nobody may; an administrator who is refused is told their account id to pass on. An installation with one organization needs nothing set: its administrators may. Ids rather than usernames, because an administrator can rename accounts in their own organization. |
