...
- **RabbitMQ Integration**: Every publish waits for a publisher confirm and is sent `mandatory`, so a message the broker cannot route is a failure rather than a silent success. Proven against a real broker in `tests/connector/broker_test.go` and `internal/app/rabbitmq_broker_test.go`.
  ...
  - **External Task Bridge**: Publish a topic's External Tasks to a RabbitMQ exchange, for workers that consume from a queue, each locked for as long as the bridge's `lock_seconds` gives its worker (five minutes unless set). A task the broker does not take, or does not confirm in time, is handed back at once. **Off unless configured** with `METIS_RABBITMQ_BRIDGES`.
...
| `METIS_PLATFORM_ADMINS` | The **platform administrators**: account ids, comma-separated, of the administrators who may install, import, switch on or off and remove connector manifests, and add, change and remove connector templates, on an installation with more than one organization. Both are installation-wide. A manifest is installation-wide — a step in any organization that names its key runs it, with that organization's credentials — and roles are global, so the administrator role alone would let one organization's administrator change what every other runs. Unset on such an installation, nobody may; an administrator who is refused is told their account id to pass on. An installation with one organization needs nothing set: its administrators may. Ids rather than usernames, because an administrator can rename accounts in their own organization. |
| `METIS_ALLOW_BUILTIN_CONNECTOR_OVERRIDE` | Let a connector manifest take the key of a connector built into Metis (`http-json`, `slack-message`, `email-smtp`, `sendgrid-email`, `discord-message`, `ms-teams-message`, `rabbitmq-publish`, `sql-query`). **Off by default**: such a manifest replaces the built-in in every step, in every organization, that uses it, so installing one is refused with a 400 naming the key. A manifest installed under a built-in's key before this setting existed keeps answering; removing it hands the key back to the built-in. |
...
| `METIS_RABBITMQ_CONFIRM_TIMEOUT` | How long a RabbitMQ publish — by the external-task bridge, by the consumer to its dead-letter queue, or by a *RabbitMQ Publisher* step — waits for the broker to confirm it (default `10s`). One not confirmed in time is treated as refused: the bridge hands the task back and publishes on a new channel, a step fails and is retried. It used to wait for ever, so a broker that took a publish and never answered held a bridge, or a job worker, until a restart. |
