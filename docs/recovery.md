...
| ~~AMQP bridge~~ | **Not a problem, on inspection.** Two replicas consuming one queue are competing consumers — each message is delivered to exactly one — and the external-task bridge polls `FetchAndLock`, which takes `FOR UPDATE SKIP LOCKED`. The per-process registry correctly answers "is this bridge running *here*". The cost is duplicated polling, not duplicated work — as long as a task is completed within its bridge's lock (`lock_seconds`, five minutes unless set). Past that it is published again, by whichever replica polls first, as it would be with one. Both have been started since 2026-09-26, when `METIS_RABBITMQ_BRIDGES` or `METIS_RABBITMQ_CONSUMERS` names them; before that nothing started either. Give every replica the same list. |
