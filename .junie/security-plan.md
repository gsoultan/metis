...
| Tenant derivation | `server/interceptors/tenant/resolver.go` | Derived from the authenticated principal loaded from the database, not from `X-Organization-ID`. An OIDC sign-in reaches it as the account linked to the token's issuer and subject, admitted to the organizations `METIS_OIDC_ORGANIZATION_CLAIM` names in that token (`impl/identity_sign_in.go`); `ErrUnresolvedTenant` refuses any principal that is not an account. |
...
| **Since the plan** | An audit against the roadmap on 2026-09-25 found and fixed eight more P0s: the setup wizard stayed open on container deployments, gRPC listened unauthenticated by default, an administrator could manage another organization's accounts, a business rule could run another tenant's decision table, and four reliability holes. See the roadmap's session log. `ENCRYPTION_KEY` could not be rotated — nothing re-encrypted sealed data under a new key, so a leaked key could not be retired. It can now: `ENCRYPTION_KEY_PREVIOUS` for reading and `metis --reseal` (`docs/runbooks.md`, "Rotating secrets"). |
