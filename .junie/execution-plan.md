...
| **3** Integration platform | partial — 3.5 (external-task protocol over HTTP + Go SDK + `docs/integration.md`) landed earlier. **Idempotency is done**: every outbound service call is recorded before it is made and completed with its response after, so a retry after a failed commit reuses the response instead of calling again, and every call carries a stable `Idempotency-Key` derived from (instance, node, iteration). The connector manifest (3.3), circuit breaker, DLQ, per-instance rate limiting and the signed webhook receiver are not started. Retry backoff is exponential with jitter. |
