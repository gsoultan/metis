...
| **3** Integration platform | **done** — 3.5 (external-task protocol over HTTP + Go SDK + `docs/integration.md`) landed earlier. **Idempotency is done**: every outbound service call is recorded before it is made and completed with its response after, so a retry after a failed commit reuses the response instead of calling again, and every call carries a stable `Idempotency-Key` derived from (instance, node, iteration). Retry backoff is exponential with jitter; a per-target circuit breaker stops an unhealthy downstream filling the job pool; and `rate_limit_per_minute` on a connector instance or a node holds calls to a partner's quota, deferring rather than failing so a quota cannot exhaust an instance's retries. **The dead-letter path is complete**: a job that exhausts its retries raises an incident, and the new incident inbox lists what failed, explains the cause in words an operations person can act on, and calls the replay the engine already supported — `useResolveIncident` existed and nothing in the interface had ever called it, so failed work was invisible and only recoverable by a database write. **The signed webhook receiver (3.6) is done**, management screen included: `POST /api/v1/hooks/{token}`, HMAC-SHA256 over the raw body checked in constant time before anything is parsed, delivery-ID dedup so a sender's retries do not move a process twice, and a FEEL expression over the payload for correlation. **The connector manifest (3.3) has landed in its core form**: a connector is a YAML or JSON document — auth, request templates, response mapping and error rules, all expressed in FEEL — parsed, validated at install, and executed by `connectors.RunManifest`, which the connector service consults before its built-in Go executors. **OpenAPI import is done too**: `connectors.ImportOpenAPI` turns a specification into one manifest per operation — path placeholders into input templates, parameters and request body into an input schema, response schema into outputs, security into auth, plus the two failures every API has. Manifests are **persisted and reachable**: installed over `POST /api/v1/connector-manifests` (manifest or OpenAPI), stored in `connector_manifests`, read per call so a replica never serves a stale catalogue, and managed from the Connectors page. What remains of 3.3 is generating the designer's property form from `input_schema`. **Phase 3 is complete.** |
| **4** BPMN × DMN | **done** — engine-side work landed (boundary events, repeating timers, multi-instance, ad-hoc activation, decision-driven routing proven by test). **4.3.6 (decision audit) is done**: every evaluation records the table, its version, the rule IDs that applied, the inputs and the outputs on the instance timeline, and the UI renders it. Governance §4.4 is enforced for deletion — a decision a running or suspended instance can still reach cannot be deleted. **4.3.2 (decision-driven assignment) is done**: a user task can name a table that decides its assignee, candidate users and groups, priority and due date, with the choice recorded on the timeline — an approval matrix changes when the org changes, which is far more often than the process does. **Half of 4.3.5 is done**: the editor flags unreachable and duplicate lines, shows which rules fired on a trial run, and now finds *coverage gaps* — combinations of inputs no line matches, which is how a table quietly returns nothing and a process carries on with an empty variable. **4.3.4 (impact view) is done**: `GET /api/v1/decisions/{id}/impact` reports which processes consult a table — naming the steps, counting the instances still running under it — and the editor shows it beside the table being changed. It sees assignment tables as well as business rule tasks. **4.3.5 is done**: examples are stored with the table (migration 6), run against it from the editor, and reported per example with what was expected, what happened and which line decided. **4.3.3 (the DRG view) is done**: the decision list shows the dependency chain in evaluation order, and flags both failures a flat list hides — a cycle, which makes every process touching it fail at runtime, and a dependency on a decision that no longer exists. **4.3.1 is done**: one palette item drops a decision table and the gateway that branches on it, already wired — the recommended shape made no harder than the unrecommended one. **Phase 4 is complete.** |
| **5** UI/UX + upgrade | partial — everything verifiable without a browser is done. 5.2.f (TypeScript 7) landed, plus the designer rebuild, Connect RPC v2, list pagination and a bundle budget. **5.4.C (Decision Editor) is done** bar the DRG view: the grid takes paste from a spreadsheet, keyboard navigation and fill-down, validates conditions as they are typed, flags unreachable and duplicate lines, and states the hit policy in words instead of hiding it behind Expert Mode. **The 5.2 table is stale**: Mantine 9, Vite 8, TypeScript 7, TanStack Router 1.170, Query 5.101, TanStack Form, Tailwind 4 and `postcss-preset-mantine` are all in, and Emotion is gone. Vitest is deliberately not adopted — `bun test` runs 133 tests and adding a second runner would be two ways to do one thing. **Task Inbox urgency (5.4.A) is done**: overdue and priority are one tested computation shown as visual weight, and the list opens in the order somebody should work. **TanStack Virtual is in**, applied to the task table above a hundred rows — below that the ordinary path is untouched. **Query tuning is done**: the client had no `staleTime` at all, so all 27 queries refetched on every mount and every window focus; each resource now has a stale time matched to how often it actually changes, and window-focus refetching is off because SSE is how this app learns something changed. The one `refetchInterval` in the codebase is a documented 30-second SSE fallback, so "no polling loops" was already met. Still missing: `vite-plugin-pwa`, `babel-plugin-react-compiler`, and i18n — all three want a browser to land safely. |
...
| :-- | :-- |
| `.tsx` files | 78 |
| Files with any `aria-label` / `role` | 29 of 90 — every icon-only control is labelled, and `eslint-plugin-jsx-a11y` now blocks new ones (figure was **1** when this table was written) |
