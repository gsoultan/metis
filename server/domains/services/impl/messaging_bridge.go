...
func (b *externalTaskBridge) poll(ctx context.Context) {
	if err := b.connect(); err != nil {
		b.logger.Error().Err(err).Dur("retryIn", b.pollInterval).
...
func (b *externalTaskBridge) forward(ctx context.Context, tasks []*entities.ExternalTask) {
	for i, task := range tasks {
		...
		// A channel that missed a confirm cannot be trusted with the next
		// one: the broker's answer about the lost message may still come, and
		// be read as its answer about another. The rest of this round goes
		// back unpublished, and the next round publishes on a new channel.
		b.releaseUnpublished(ctx, tasks[i+1:], err)
		b.link.dropChannel()
		b.publisher = nil
		return
	}
...
func (b *externalTaskBridge) releaseUnpublished(ctx context.Context, tasks []*entities.ExternalTask, cause error) {
	...
	b.logger.Warn().Int("tasks", len(tasks)).
		Msg("Tasks fetched in the same round as one the broker did not confirm were handed back unpublished")
}
